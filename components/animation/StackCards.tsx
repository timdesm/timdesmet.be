"use client";
import { useLayoutEffect, useEffect, useRef, Children, isValidElement, useState } from "react";
...
export default function StackCards({ className, offset, pin = true, scrub = true, durationMul = 0.5, children, stackName = "services-stack", initialDelay = 200 }: StackCardsProps) {
  const wrapperRef = useRef<HTMLDivElement | null>(null);
  const offsetRef = useRef<HTMLDivElement | null>(null);
  const [renderStack, setRenderStack] = useState(false); // render control

  // delay the rendering
  useEffect(() => {
    const timer = setTimeout(() => setRenderStack(true), initialDelay);
  ...
  }, [pin, scrub, durationMul, children, renderStack]);
